...
export class AddContractModalComponent implements OnInit {
  ...
  private applyPrefillData(data: Record<string, any>): void {
    ...
    // 1. Flat structure from area: { contactName: "...", areaBuilding: "..." }
    // 2. Nested structure from customer: { generalDetails: { contactName: "..." } }
    let sourceData = data;

    // If data has generalDetails key, use that (customer quotation format)
    if (data['generalDetails']) {
      console.log('[AddContractModal] Detected nested structure (customer quotation)');
      sourceData = { ...data['generalDetails'] }; // Flatten it
    } else {
      console.log('[AddContractModal] Detected flat structure (area quotation)');
    }

    // field ใน generalDetailForm ที่รับค่าจาก area หรือ customer ได้
    const generalFields = [
      'branch',
      'areaBuilding',
      'areaFloor',
      ...
      'contactName',
      'contactPhone',
      'businessName',    // ← Added for customer
      'customerId',      // ← Added for customer
    ];
    ...
    if (Object.keys(generalPatch).length > 0) {
      ...
      console.log('[AddContractModal] Form values after patch:', this.generalDetailForm.value);
    }

    // ✅ ถ้ามี monthlyRent ส่งไปที่ conditionsForm → rentRate ด้วย
    if (sourceData['areaMonthlyRent']) {
      const rent = Number(sourceData['areaMonthlyRent']);
